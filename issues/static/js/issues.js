...
(function() {
  ...
  Backbone.CollectionIntersection = (function() {
    CollectionIntersection.extend = Backbone.Model.extend;

    _.extend(CollectionIntersection.prototype, Backbone.Events);

    CollectionIntersection.prototype.parents = [];

    CollectionIntersection.prototype.operations = 0;

    function CollectionIntersection(options) {
      var parent, _i, _len, _ref;
      if (options == null) {
        options = {};
      }
      if (!_.isArray(options.parents)) {
        throw 'No options.parent passed to the CollectionIntersection';
      }
      _ref = options.parents;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        parent = _ref[_i];
    ...
    CollectionIntersection.prototype.addParent = function(collection, options) {
      var model, _i, _len, _ref, _results;
      this.parents.push(collection);
      collection.on('add', this._onParentAdd, this);
      collection.on('remove', this._onParentRemove, this);
      collection.on('reset', this._onParentReset, this);
      collection.on('change', this._onParentChange, this);
      collection.on('dispose', this._onParentDispose, this);
      collection.on('loading', this._onParentLoading, this);
      collection.on('ready', this._onParentReady, this);
      if (!(options && options.refresh === false)) {
        _ref = this.child.models;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          model = _ref[_i];
          if (!this.filter(model)) {
            _results.push(this.child.remove(model, {
              subset: this
            }));
          } else {
            _results.push(void 0);
    ...
    CollectionIntersection.prototype.removeParent = function(collection, options) {
      var index;
      index = this.parents.indexOf(collection);
      if (index === -1) {
        return;
      }
      collection.off(null, null, this);
      return this.parents.splice(index, 1);
    };

    CollectionIntersection.prototype.setChild = function(collection) {
      var _ref;
      if ((_ref = this.child) != null) {
        _ref.off(null, null, this);
      }
      this.child = collection;
      this.child.on('add', this._onChildAdd, this);
      this.child.on('remove', this._onChildRemove, this);
      this.child.on('reset', this._onChildReset, this);
      this.child.on('dispose', this.dispose, this);
      this.child.filterer = this;
      return this.refresh();
    ...
    CollectionIntersection.prototype.filter = function(model) {
      var parent, _i, _len, _ref;
      _ref = this.parents;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        parent = _ref[_i];
        if ((parent.get(model.cid)) === void 0) {
          return false;
        }
      }
    ...
    CollectionIntersection.prototype.refresh = function() {
      var model, models;
      if (this.parents.length > 0) {
        models = (function() {
          var _i, _len, _ref, _results;
          _ref = this.parents[0].models;
          _results = [];
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            model = _ref[_i];
            if (this.filter(model)) {
    ...
    CollectionIntersection.prototype._replaceChildModel = function(parentModel) {
      var childModel, index;
    ...
    CollectionIntersection.prototype._onParentAdd = function(model, collection, options) {
      if (options && options.subset === this) {
        return;
      }
      if (this.filter(model)) {
    ...
    CollectionIntersection.prototype._onParentRemove = function(model, collection, options) {
      return this.child.remove(model, options);
    };

    CollectionIntersection.prototype._onParentReset = function(collection, options) {
      return this.refresh();
    };

    CollectionIntersection.prototype._onParentChange = function(model, changes) {};

    CollectionIntersection.prototype._onParentLoading = function() {
      if (this.operations++ === 0) {
        return this.child.trigger('loading');
      }
    };

    CollectionIntersection.prototype._onParentReady = function() {
      if (--this.operations === 0) {
        return this.child.trigger('ready');
      }
    };

    CollectionIntersection.prototype._onChildAdd = function(model, collection, options) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        parent = _ref[_i];
        _results.push(parent.add(model));
      }
      return _results;
    };

    CollectionIntersection.prototype._onChildRemove = function(model, collection, options) {
      var parent, _i, _len, _ref, _results;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        _results.push(parent.remove(parent.get(model.cid)));
      }
      return _results;
    };

    CollectionIntersection.prototype._onChildReset = function(collection, options) {
      if (options && options.subset === this) {
        return;
      }
      this.parents.add(this.child.models);
      return this.refresh;
    };

    CollectionIntersection.prototype.dispose = function() {
      var parent, prop, _base, _i, _j, _len, _len1, _ref, _ref1;
      if (this.disposed) {
        return;
      }
  ...
  IssueListItemView = (function(_super) {
    ...
    IssueListItemView.prototype.selected = function(state) {
      if (state == null) {
        return this.$('input[type=checkbox]').get(0).checked;
      } else {
        return this.$('input[type=checkbox]').get(0).checked = state;
      }
    };
  ...
  IssueListView = (function(_super) {
    ...
    IssueListView.prototype.events = {
      'click .mark-issues-completed-button': function(evt) {
      ...
      'click .select-issues-button': function(evt) {
        var all_selected, child, cid, _ref8, _ref9;
        evt.preventDefault();
        all_selected = true;
        _ref8 = this.children;
        for (cid in _ref8) {
        ...
        for (cid in _ref9) {
          child = _ref9[cid];
          child.selected(!all_selected);
        }
        return this.$('.select-issues-button').toggleClass('deselect-all', !all_selected);
      },
      'click .label-issues-button': function(evt) {
        var child, cid, intersection, label_collections, _ref8,
          _this = this;
        if (this.labelSelector && this.labelSelector.isVisible()) {
          return this.labelSelector.hide();
        } else {
          label_collections = [];
          _ref8 = this.children;
          for (cid in _ref8) {
          ...
          this.labelSelector = new DropdownLabelListView({
            model: app.labelCollection,
            selected: intersection.child
          });
          console.log(intersection.child.models);
          this.labelSelector.render();
          this.$el.append(this.labelSelector.el);
          this.labelSelector.show(this.$('.label-issues-button'));
          return this.labelSelector.on('hide', function() {
            _this.labelSelector.remove();
            intersection.dispose();
            return _this.labelSelector = null;
          });
        }
      }
    };
    ...
    IssueListView.prototype.remove = function() {
      if (this.labelSelector) {
        this.labelSelector.hide();
      }
      return IssueListView.__super__.remove.call(this);
    };
  ...
  DropdownLabelListView = (function(_super) {
    ...
    function DropdownLabelListView(options) {
      _.extend(this, Backbone.Events);
      DropdownLabelListView.__super__.constructor.call(this, options);
    }
    ...
    DropdownLabelListView.prototype.initialize = function(options) {
      ...
      this.blurCallback = function(evt) {
        ...
        if (_this.triggerButton && jQuery(evt.target).isOrIsChildOf(_this.triggerButton)) {
    ...
    DropdownLabelListView.prototype.updateChildren = function() {
      var child, cid, _i, _len, _ref15, _results;
      _ref15 = this.children;
      _results = [];
      for (child = _i = 0, _len = _ref15.length; _i < _len; child = ++_i) {
        cid = _ref15[child];
    ...
    DropdownLabelListView.prototype.show = function(parent) {
      var parent_pos,
        _this = this;
      this.triggerButton = jQuery(parent).get(0);
      this.filterField.val('');
      this.filter('');
      this.$el.show();
  ...
  LabelContextMenu = (function(_super) {
    ...
    function LabelContextMenu() {
      ...
      _ref15 = LabelContextMenu.__super__.constructor.apply(this, arguments);
      return _ref15;
    }
  ...
  AppRouter = (function(_super) {
    ...
    AppRouter.prototype.initialize = function(config) {
      ...
      this.labelCollection.url = '/api/labels';
